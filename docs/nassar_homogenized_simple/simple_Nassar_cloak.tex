...
The density is set to~\eqref{eq:rhoeff} computed from the cell center coordinates.
...
\begin{figure}[H]
  ...
  \caption{Cartesian cell decomposition of the circular cloak at
...
Refinement x5 is applied near the circle. 
(This is not the cell mesh from the experiment above,
just the FEM mesh)
...
right-boundary distortion yields convergence exponent $p \approx 1.35$ and asymptote
$\mathcal{D}_\infty \approx 19\%$ (Fig.~\ref{fig:ceff_convergence}).
This finding indicates that a
...
limitation of the continuous cloak (probably related to the discontinuity near the circle).
